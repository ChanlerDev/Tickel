...
4. 浏览器打开 Web 收据页面
5. 用户在页面内预览收据，必要时微调展示参数
6. 用户点击 "Download PNG" 获取高清收据图片
...
Web 首页采用工作台布局：
- 左侧为主预览区域，居中展示收据卡片，并承载导出操作
- 右侧为参数控制区，用于调整模板、项目标题、日期、模型和 token/cost 展示字段
- 当 URL 中存在多模型明细时，右侧展示对应模型条目的编辑表单，预览区实时反映修改结果
